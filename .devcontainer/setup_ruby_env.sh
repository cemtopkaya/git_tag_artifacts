...
check_redmine_isup(){
    ...
    while ! curl -s -o /dev/null -w '%{http_code}' $url; do 
        echo "Redmine için PUMA sunucusuna ($url) bağlanılamadı, 4sn bekleyip tekrar denenecek..."
        sleep 4; 
    done;
}
...
create_symlink_of_plugin(){
    ...
    ln -sf -v /workspace /usr/src/redmine/plugins/git_tag_artifacts && echo "link created" || echo "link creation failed"
}
